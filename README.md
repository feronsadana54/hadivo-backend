...
- Face Recognition Enrollment Foundation v1.6.0 menambahkan tabel `user_face_profiles` per (tenant, user), endpoint enroll / reset / get profil wajah dengan consent eksplisit, penyimpanan file enrollment di disk lokal yang `.gitignore`-d, validasi format JPEG/PNG + size cap (bukan deteksi wajah), reset oleh admin yang membersihkan referensi DB dan best-effort menghapus file lokal, kolom Wajah di halaman web `Anggota`, dan layar `Daftar Wajah` di mobile. Versi ini belum melakukan face detection / matching / liveness / anti-spoofing dan tidak memblokir clock-in / clock-out.
